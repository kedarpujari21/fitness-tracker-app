...
public class JwtUtils {
    ...
    public String getJwtFromHeader(HttpServletRequest request){
        ...
        if(bearerToken != null && bearerToken.startsWith("Bearer ")){
            return bearerToken.substring(7); // return token and ignores first 7 letters i.e. Bearer ....
        }
        return null;
    }
    ...
    public String generateToken(String userId, String role){
        return Jwts.builder()
                .subject(userId) // to whom the token belongs to
                .claim("roles", List.of(role))
                .issuedAt(new Date()) // current date
                .expiration(new Date(new Date().getTime() + jwtExpirationMs)) //expiration date and time
                .signWith(key())
                .compact();
    }
